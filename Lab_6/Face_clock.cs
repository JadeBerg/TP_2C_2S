...
namespace Lab_6
{
    public partial class Face_clock : Form
    {
        ...
        Graphics cg;
        public Face_clock() { }
        public Face_clock(Color BackGroundColor, Color HourHandColor, Color MinuteHandColor, Color SecondHandColor)
        {
            InitializeComponent();
        ...
        private void t_Tick(object sender, EventArgs e)
        {
            ...
            cg.DrawLine(new Pen(SecondHandColor, 2f), new Point(cx, cy), new Point(handCoord[0], handCoord[1]));
            //draw minutes hand   
            handCoord = msCoord(mm, minHAND);
            cg.DrawLine(new Pen(MinuteHandColor, 3f), new Point(cx, cy), new Point(handCoord[0], handCoord[1]));
            //draw hours hand   
            handCoord = hrCoord(hh % 12, mm, hrHAND);
            cg.DrawLine(new Pen(HourHandColor, 3f), new Point(cx, cy), new Point(handCoord[0], handCoord[1]));
            //load the bitmap image   
            pictureBox1.Image = bmp;
            //display time in the heading   
            this.Text = "Струна Виктор " + hh + ":" + mm + ":" + ss;
            cg.Dispose();
        }
